...
## نقاط النهاية API المتوفرة

### الصفحة الرئيسية
```http
GET /
```
تعرض قائمة جميع نقاط النهاية المتاحة مع وصف موجز لكل منها.

### البحث في متن الحديث
```http
GET /api/search?q=:query
```
البحث في نص الحديث باللغة العربية أو الإنجليزية.

### البحث عن الأحاديث حسب الراوي
```http
GET /api/search/narrator/:name
```
